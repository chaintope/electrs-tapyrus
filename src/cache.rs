...
mod tests {
    ...
    fn gen_block_hash(seed: u8) -> BlockHash {
        let bytes: Vec<u8> = (seed..seed + 32).collect();
        BlockHash::hash(&bytes[..])
    }

    fn gen_tx_hash(seed: u8) -> Txid {
        let bytes: Vec<u8> = (seed..seed + 32).collect();
        Txid::hash(&bytes[..])
    }
    ...
    fn test_blocktxids_cache_hit_and_miss() {
        let block1 = gen_block_hash(1);
        let block2 = gen_block_hash(2);
        let block3 = gen_block_hash(3);
        let txids = vec![gen_tx_hash(4), gen_tx_hash(5)];
